print("Скрипт розпочав роботу з новим токеном...")
...
async def edit_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    context.user_data['photos'] = []
    await update.callback_query.message.reply_text("🔧 Режим редагування. Введіть нову марку або натисніть кнопку:", reply_markup=ReplyKeyboardMarkup(SKIP_KEY, resize_keyboard=True))
    return MAKE
...
async def get_make(update: Update, context: ContextTypes.DEFAULT_TYPE): return await step_handler(update, context, 'make', MODEL, "Введіть модель:")
async def get_model(update: Update, context: ContextTypes.DEFAULT_TYPE): return await step_handler(update, context, 'model', YEAR, "Введіть рік:")
async def get_year(update: Update, context: ContextTypes.DEFAULT_TYPE): 
    k = GEARBOX_KEYS + (SKIP_KEY if context.user_data.get('is_edit') else [])
    return await step_handler(update, context, 'year', GEARBOX, "Оберіть КПП:", ReplyKeyboardMarkup(k, resize_keyboard=True))
...
async def get_district(update: Update, context: ContextTypes.DEFAULT_TYPE): return await step_handler(update, context, 'district', TOWN, "Місто/село:")
...
async def get_desc(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Надішліть фото (до 10) та натисніть /done", reply_markup=ReplyKeyboardMarkup(SKIP_KEY, resize_keyboard=True))
    return PHOTOS
...
async def final_post(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message.text == "Так":
        ...
        media = [InputMediaPhoto(context.user_data['photos'][0], caption=context.user_data['summary'], parse_mode=ParseMode.HTML)]
        for p in context.user_data['photos'][1:10]: media.append(InputMediaPhoto(p))
        
        msgs = await context.bot.send_media_group(chat_id=CHANNEL_ID, media=media)
        m_ids = ",".join([str(m.message_id) for m in msgs])
        
        conn = sqlite3.connect(DB_PATH)
        conn.execute('INSERT INTO ads (user_id, msg_ids, details) VALUES (?, ?, ?)', (update.effective_user.id, m_ids, context.user_data['summary']))
        conn.commit()
        conn.close()
...
async def my_ads(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn.close()
    if not rows: return await update.message.reply_text("У вас немає оголошень.")
    for r_id, txt in rows:
        kb = [[InlineKeyboardButton("📝 Редагувати", callback_data=f"edt_{r_id}"), InlineKeyboardButton("🗑 Видалити", callback_data=f"del_{r_id}")]]
